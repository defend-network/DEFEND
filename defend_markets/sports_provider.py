"""DEFENDmarkets-owned The Odds API provider.

Table tennis events, h2h odds, scores and live state from The Odds API v4.
Markets-owned replacement for the legacy ``defend_sports`` provider; imports
only Markets-owned primitives (``defend_markets.feeds`` HTTP helpers and
``defend_markets.sports_domain`` DTOs). No legacy application dependency.

Only real API data is emitted: HTTP failures surface as provider errors and
missing credentials must be reported by the caller before polling
(UNCONFIGURED).
"""
...
class RawProviderEvent:
    ...
    Stored verbatim so normalization bugs can be reproduced. Raw/vendor field
    names never leave this model; canonical consumers reference it only through
    ``raw_event_ref``.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.provider_event_id, str) or not self.provider_event_id.strip():
            raise ValueError("provider_event_id must not be blank")
        if not isinstance(self.payload, Mapping):
            raise ValueError("payload must be a mapping")
        if not isinstance(self.observed_at, datetime) or self.observed_at.tzinfo is None:
            raise ValueError("observed_at must be a timezone-aware datetime")
        if self.received_at is not None and (
            not isinstance(self.received_at, datetime) or self.received_at.tzinfo is None
        ):
            raise ValueError("received_at must be a timezone-aware datetime")
        if self.display_name is not None and (not isinstance(self.display_name, str) or not self.display_name.strip()):
            ...
            for bookmaker in bookmakers:
                ...
                for market in markets:
                    ...
                    for outcome in outcomes:
                        ...
                        match_odds.append(
                            OddsObservation(
                                source=SourceRef(provider=self.provider_name, external_id=book_key),
                                event_external_id=match_id,
