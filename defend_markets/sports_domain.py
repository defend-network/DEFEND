"""DEFENDmarkets-owned sports/odds domain models.

Markets-owned DTOs replacing the legacy ``defend_sports`` domain models. These
carry provider provenance through the live collection pipeline (event identity,
market identity, selection identity, odds snapshots, live observations) so no
legacy application package is imported by canonical Markets code.

Nothing here performs network I/O or persistence.
"""
