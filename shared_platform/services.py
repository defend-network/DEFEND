...
def validate_deployment(
    ...
    ordered_contexts = validate_applications(contexts)
    ...
    for service in services:
        if service.application_id not in context_by_id:
            raise ValueError(
                f"{service.application_id} service has no application context"
            )
    ...
    for route in routes:
        if route.application_id not in context_by_id:
            raise ValueError(
                f"{route.application_id} route has no application context"
            )
    ...
    if set(route_by_id) != set(context_by_id):
        raise ValueError("deployment requires exactly one route per application")
