...
export function KnowledgePanel(){
  ...
  return <section className="knowledge-panel">
    ...
    {discovery.length>0&&<article className="card"><h3>Discovered candidates</h3><table className="data-table"><thead><tr><th>File</th><th>Type</th><th>SHA</th><th>State</th><th>Actions</th></tr></thead><tbody>{discovery.map(doc=>{const id=String(doc.discovery_id);return <tr key={id}><td>{String(doc.filename)}</td><td>{String(doc.candidate_source_type||"UNKNOWN")}</td><td>{String(doc.file_sha256||"").slice(0,10)}</td><td><span className="pill">{String(doc.state)}</span></td><td>{(doc.state==="DISCOVERED"||doc.state==="CLASSIFIED")&&<><button onClick={()=>setEdit(edit?.id===id?null:{id,source_type:String(doc.candidate_source_type||""),manufacturer:"",model:"",model_series:"",edition:""})}>Classify</button> <button onClick={()=>approve(id)}>Approve &amp; Index</button> <button onClick={()=>block(id)}>Block</button></>}</td></tr>})}</tbody></table></article>}
